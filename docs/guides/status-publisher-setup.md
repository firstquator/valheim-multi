...
같은 Gist 안에 `valheim-history.json` 이 하나 더 생긴다. **손으로 만들 필요는
없다.** 퍼블리셔가 처음 돌 때 알아서 만든다.

두 파일로 나눠 두는 이유는 성격이 다르기 때문이다. 상태는 30초마다 통째로
덮어쓰는 값이고, 기록은 시간 단위로 쌓이는 값이다. 사이트도 상태는 30초마다
받지만 기록은 서버 탭을 열 때 한 번만 받는다.
...
- [ ] `valheim-history.json` 이 생겼고 `hours` 안에 시각 칸이 하나 이상 있다
- [ ] 같은 파일의 `deaths` 와 `raids` 가 목록이다 (아무도 안 죽었으면 비어 있어도 맞다)

`deaths` 와 `raids` 는 서버 로그에서 읽는다. 죽음은 `Got character ZDOID from
<이름> : 0:0`, 습격은 `Random event set:<이름>` 이다. 둘 다 실물 로그로 확인한
패턴이다.

퍼블리셔는 30초마다 12시간치 로그를 통째로 다시 읽으므로 같은 죽음이 수없이
다시 나온다. 시각과 이름을 열쇠로 합쳐 두 번 세지 않는다. 그래서 `docker logs`
에 `--timestamps` 를 붙인다. 컨테이너 안쪽 시각은 지역 시간이라 시간대를 알 수
없지만 이쪽은 언제나 UTC 다.

기록은 30일치만 남기고 오래된 칸을 버린다. 퍼블리셔가 시작할 때 기존 기록을
한 번 읽어 이어 쌓으므로, 프로세스를 다시 띄워도 과거가 사라지지 않는다.
읽기에 실패하면 그 주기에는 기록을 올리지 않는다. 빈 기록으로 덮어쓰는 것이
잠시 못 쌓는 것보다 나쁘기 때문이다.
